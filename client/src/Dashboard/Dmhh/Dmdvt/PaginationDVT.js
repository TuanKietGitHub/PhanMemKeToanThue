...
const PaginationDVT = () => {
    ...
    const pages = [];
    for( let i = 1 ; i <= Math.ceil(List_DVT.length/sizePageModal) ; i++ ) {
        pages.push(i);
    }
                        ...
                        <tbody className='table-body'>
                            { currentItems.map((item , index)  => (
                                <tr key={index}>
                                    ...
                                    <td className ="filterable-cell" onClick={() => dataOnclick(item.Dvt)}>{item.Dvt}</td>
                                </tr>
                            ))
                            }
                        </tbody>
